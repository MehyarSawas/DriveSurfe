...
2. **Content-Security-Policy added** in two places: `Application.php`'s security-header middleware sets `default-src 'none'; frame-ancestors 'none'` on all `/api` JSON/binary responses (they never render HTML, so this is maximally strict); the SPA's policy lives in a **`<meta http-equiv="Content-Security-Policy">` tag in `frontend/src/index.html`** (`script-src 'self'; object-src 'none'; base-uri 'self'`). **Two real production incidents to know about before touching this:**
   - `base-uri 'none'` broke Angular's `<base href="/">` (relative bundle URLs 404'd on deep routes → white screen). Must stay `'self'`.
   - **Any `script-src 'self'` policy requires `optimization.styles.inlineCritical: false` in `angular.json`** (it's set — don't remove it). Angular's critical-CSS inliner rewrites the stylesheet link to `media="print" onload="this.media='all'"`; CSP blocks the inline `onload` handler, the media attribute never flips, and the entire app renders unstyled (styles apply only to print). This cost several debugging rounds in production — the CSP header was wrongly blamed and moved to a meta tag before the actual cause was found in the build output (`dist/.../index.html`). When changing CSP or build optimization settings, always inspect the **built** `index.html` for inline handlers/scripts, and note `ng serve` behaves differently from the production build.
   - **`script-src` must include `'unsafe-eval'`** (plus `'wasm-unsafe-eval'`): OpenCV.js is an embind build that generates its JS/C++ glue via `new Function()` at runtime. Without it, OpenCV init throws on every source and the scanner shows "Basic mode — all sources failed" (verified by loading the vendored file under the exact CSP in headless Chromium). If you ever replace OpenCV with a `DYNAMIC_EXECUTION=0` build, `'unsafe-eval'` can be dropped.
   `frame-ancestors` can't be expressed in a meta CSP, so framing is blocked via `X-Frame-Options: DENY` (`.htaccess`, FilesMatch on index.html).
