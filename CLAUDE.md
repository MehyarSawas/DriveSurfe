...
2. **Content-Security-Policy added** in two places: `Application.php`'s security-header middleware sets `default-src 'none'; frame-ancestors 'none'` on all `/api` JSON/binary responses (they never render HTML, so this is maximally strict); the SPA's policy lives in a **`<meta http-equiv="Content-Security-Policy">` tag in `frontend/src/index.html`** (`script-src 'self'; object-src 'none'; base-uri 'self'`), NOT in `.htaccess`. Three production incidents shaped this — do not "clean it up" back into a header: (a) `base-uri 'none'` broke Angular's `<base href="/">` (relative bundle URLs 404'd on deep routes → white screen); (b) resource directives (`style-src`/`font-src`/etc.) corrupted styles on the host; (c) bisection proved the **host mangles the CSP response header itself** — even the minimal header-based policy broke stylesheet loading, and removing the header (identical policy, meta tag) fixed it. `frame-ancestors` can't be expressed in a meta CSP, so framing is blocked via `X-Frame-Options: DENY` in `.htaccess`. `ng serve` doesn't apply `.htaccess` locally — test any CSP change against the real deploy.
