...
2. **Content-Security-Policy added** in two places: `Application.php`'s security-header middleware sets `default-src 'none'; frame-ancestors 'none'` on all `/api` JSON/binary responses (they never render HTML, so this is maximally strict); `backend/public/.htaccess` sets a deliberately **minimal** SPA policy: `script-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'`. Two production incidents shaped this: `base-uri 'none'` broke Angular's `<base href="/">` (relative bundle URLs 404'd on deep routes → white screen), and a full resource policy (`default-src`/`style-src`/`font-src`/`img-src`) corrupted styles/fonts on the production host. Script execution is the actual attack vector and it stays locked (no CDN/injected/inline scripts); do NOT re-add resource directives without testing on the real deployed host — `ng serve` doesn't go through Apache, so `.htaccess` CSP never applies locally.
